...
outdirPrefix = '/home/pavd/Desktop/mt_omnistereo/MasterThesis_Angles_related/Textured_angles_Experiment2/CalibrationFiles_Experiment2_TexturedSurfaces/'
...
def angle_between(p0,p1,p2):
    ...
    print("="*100) 
    print(' Angle between Cam0 and Cam1 is : ', np.degrees(angle1), 'degrees')
    print(' Angle between Cam1 and Cam2 is : ', np.degrees(angle2), 'degrees')
    print(' Angle between Cam2 and Cam0 is : ', np.degrees(angle3), 'degrees')    
    print("="*100) 
...
def physicalCam(path,Cam2_cx,Cam2_cy,Cam2_cz, psi1, theta1, phi1):
    # creating XML 
...
def virtCamC0a(path):
 # creating XML 
    ...
    child16 = etree.Element('rz')
    child10.append(child16)
    #child16.text = str(np.deg2rad(-150)) 
...
def virtCamC1a(path,Cam1_cx,Cam1_cy,Cam1_cz):
  # creating XML 
    ...
    child16 = etree.Element('rz')
    child10.append(child16)
    #val = np.deg2rad(-240)  #undo phy. +120 deg orientation and further -120deg
    child16.text = str(np.deg2rad(caZRotInDeg)) 
    ############################################
    root.append(child)
    s = etree.tostring(root, pretty_print=True)
...
def virtCamC0b(path):
 # creating XML 
    ...
    child16 = etree.Element('rz')
    child10.append(child16)
    #child16.text = str(np.deg2rad(-30)) # near correct
...
def angles_Cam0(cam_object0):
    ...
    for i in range(0, max_angle_deg, stepsize):
        ...
        mat_rotated_vector = np.squeeze(np.asarray(mat_rotated))
        ...
        #scene.render.filepath = '/home/pavd/Desktop/mt_omnistereo/MasterThesis_Angles_related/Textured_angles_Experiment2/C0/camop0_%02d_degree_rotation.png' %i
        ...
        physicalCam(outDir + 'C0_%02d_degree_rotation.xml' %i, 0, 0, 0, np.deg2rad(i), 0, 0)
        virtCamC0a(outDir + 'C0a_%02d_degree_rotation.xml' %i)
        virtCamC0b(outDir + 'C0b_%02d_degree_rotation.xml' %i)
        
        extrinsicsCam0(outDir +'extrinsics_%02d_degree_rotation.xml' % i, p0[0], p0[1], p0[2])
...
def angles_Cam1(cam_object1):
    ...
    for i in range(0, max_angle_deg, stepsize):
        ...
        bpy.context.object.data.type = 'PANO'
        bpy.context.object.data.cycles.panorama_type = 'FISHEYE_EQUIDISTANT'
        #scene.render.filepath = '/home/pavd/Desktop/mt_omnistereo/MasterThesis_Angles_related/Textured_angles_Experiment2/C1/camop1_%02d_degree_rotation.png' %i
        #bpy.ops.render.render(write_still=True)
        ...
        physicalCam(outDir + 'C1_%02d_degree_rotation.xml' %i ,Cam1_cx,Cam1_cy,Cam1_cz, np.deg2rad(i), 0, np.deg2rad(120))
        virtCamC1a(outDir +'C1a_%02d_degree_rotation.xml' %i,Cam1_cx,Cam1_cy,Cam1_cz)
        bpy.context.scene.update()   
...
def angles_Cam2(cam_object2):
    ...
    for i in range(0, max_angle_deg, stepsize):
        ...
        bpy.context.object.data.type = 'PANO'
        bpy.context.object.data.cycles.panorama_type = 'FISHEYE_EQUIDISTANT'
        #scene.render.filepath = '/home/pavd/Desktop/mt_omnistereo/MasterThesis_Angles_related/Textured_angles_Experiment2/C2/camop2_%02d_degree_rotation.png' %i
        #bpy.ops.render.render(write_still=True)
        ...
        physicalCam(outDir + 'C2_%02d_degree_rotation.xml' %i ,Cam2_cx,Cam2_cy,Cam2_cz, np.deg2rad(i), 0, np.deg2rad(-120))
        virtCamC2b(outDir +'C2b_%02d_degree_rotation.xml' %i,Cam2_cx,Cam2_cy,Cam2_cz)
...
path_to_obj_dir = '/home/pavd/Desktop/mt_omnistereo/MasterThesis_Angles_related/Textured_angles_Experiment2/Experiment3/'
